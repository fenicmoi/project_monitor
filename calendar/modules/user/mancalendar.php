...
if ($_SESSION['USER_PRI'] != 1) {    //กรณีไม่ใช่ admin
    $condi = " where status =0 and secretary_id='".$_SESSION['USER_LOGIN']."' ";
    $condi1 = ' where '.TB_man.".secretary_id='".$_SESSION['USER_LOGIN']."' ";
}elseif($_SESSION['USER_PRI']==1) {  //กรณีเป็น admin  แสดงข้อมูลเฉพาะคนที่อยู่ในวาระเท่านั้น
	$condi = "where status = 0  ";
}
?>
        ...
        if ($_POST['save']) {
			...
			*/
            ...
            $sql = "INSERT INTO calendar(manager_id,calendar_stdate,calendar_sttime,calendar_endate,calendar_entime,calendar_title,
			                              calendar_own,calendar_detail,calendar_location,calendar_remark,calendar_status)
					VALUES( 
				  			 $manager_id,'$calendar_stdate','$calendar_sttime','$calendar_endate','$calendar_entime',
				   			'$calendar_title','$calendar_own','$calendar_detail','$calendar_location','$calendar_remark',$calendar_status
					)";
				...
				<div class="col-md-6 well">
					<div class="input-group">
						...
						<select class='form-control' name='manager_id'  >
							...
							<?php 
                            $res[calen] = $db->select_query('SELECT * FROM '.TB_man." $condi  order by manager_order ", MYDBMS);
            while ($arr[calen] = $db->fetch($res[calen], MYDBMS)) {
                ?><option value='<?php echo $arr[calen][manager_id]; ?>' <?php if ($arr[calen][manager_id] == $arr[car][cars_member]) {
					...
					<div class="form-group form-inline">
						...
						<div class="input-group">
							<span class="input-group-addon">การแต่งกาย</span>
						...
						<div class="input-group">
							<label>สถานะ</label>
							 <label class="radio-inline">
								<input type="radio" name="calendar_status" value=0 >ไม่เปิดเผย
								</label>
								<label class="radio-inline">
								<input type="radio" name="calendar_status" value=1>เปิดเผย
							</label>
						</div>
						...
						<center>
							...
							<a class="btn btn-danger" href="?job=listdata">ยกเลิก</a>
						</center>
				...
				<div class="row">
					...
					<div class="col-md-6 well">
						...
						<div class="form-group">
						    <div class="input-group">
								<span class="input-group-addon" for="manager_id">ผู้บริหาร2:</span>
								<select class='form-control' name='manager_id'  >
									<option value=''>-- เลือกผู้บริหาร --</option>
									<?php 
						...
						<div class="form-group">
							<div class="input-group">
								<span class="input-group-addon" for="">หัวข้อภารกิจ:</span>
							    <input  type='text' name='calendar_title' value='<?php echo $arr[calen][calendar_title]; ?>' size='70' class='input-text form-control' >
							</div>
						</div>

						<div class="form-group form-inline">
							<label for="">รายละเอียดภารกิจ:</label>
							<textarea name='calendar_detail' cols='70' rows='5' class='input-textarea form-control'><?php echo $arr[calen][calendar_detail]; ?></textarea>
						</div>
						...
						<div class="form-group">
							<div class="input-group">
								<span class="input-group-addon">เจ้าของเรื่อง/โทรศัพท์</span>
						...
						<br>
						  <?php 
							$status = $arr[calen][calendar_status];
							if($status == 0){
								$notOk="checked";
							}
							if($status == 1){
						...
						<div class="input-group">
							<label>สถานะ</label>
							 <label class="radio-inline">
								<input type="radio" name="calendar_status" value=0 <?php if($status==0){ echo "checked";} ?>>ไม่เปิดเผย
								</label>
								<label class="radio-inline">
								<input type="radio" name="calendar_status" value=1 <?php if($status==1) { echo "checked";} ?>>เปิดเผย
							</label>
						</div>
						<center>
            ...
            $endate = substr($arr[calen][calendar_endate], 0, 10).' 00:00:00'; ?>
                ...
                echo "class='data-edit'";
						 ...
						 <td  align='center' valign='top'>
							...
							<?php 
							if (strtotime($stdate) != strtotime($endate)) {
						 ...
						 <td  align='left' valign='top'><?php echo $arr[calen][calendar_title]; ?> , <i class="fa fa-map-marker"></i>สถานที่ : <?php echo $arr[calen][calendar_location]; ?></td> 
							...
							<?php 
								...
								<?php }else{ ?>
									<td >
										<button type="button" class="btn btn-succcess"><i class="fa fa-eye"></i></button>	
								<?php } ?>
						</td>
