...
<!-- <div id='main'> -->
	<div class="row">
		...
		<form method=post action="?" name="webForm">
					...
					<p class='detailtext'>
						<input name="calendar_stdate" value='<?php echo $_POST['calendar_stdate'];?>' id="start-date" class="date-pick dp-applied form-control" onkeypress="javascript:return  true;">
					...
					<p class='detailtext'>
						<input name="calendar_endate" value='<?php echo $_POST['calendar_endate']?>' id="end-date" class="date-pick dp-applied form-control" onkeypress="javascript:return  true;" >
					</p>
					<p class='detailtext'>
						<!-- &nbsp;&nbsp;<input type='submit' class="btn btn-warning " value=' แสดงข้อมูล '>	 -->
						<button type="submit" class="btn btn-primary " value=' แสดงข้อมูล '><i class="fa fa-eye"></i>แสดงนัดงาน</button>
					</p>	
					<p class='detailtext'>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-warning" href='calendar/prinpreviewonday.php?sdate=<?php echo $sdate?>&endate=<?php echo $endate?>' target='_blank'><i class="fa fa-print"></i> สั่งพิมพ์</a>					
					</p>
		</form>
	  <hr>
	  <br>
	  <br> 
		  ...
		  <thead class="bg-primary">
			<tr>
				...
				<th width="15%">เจ้าของเรื่อง/โทร</th>
				<th width="20%">การแต่งกาย</th>
			</tr>
		</thead>
				...
				<?php
					...
					$sql="SELECT c.*,m.manager_order  FROM ".TB_calendar." AS c 
					      INNER JOIN manager AS m ON m.manager_id = c.manager_id where (( calendar_stdate between '".$sdate."' and '".$endate."') or ( calendar_endate between '".$sdate."' and '".$endate."') or ( calendar_stdate < '".$sdate."' and calendar_endate> '".$endate."')) and calendar_endate and calendar_status=1 ORDER BY manager_order,calendar_sttime,calendar_entime";
					//print $sql;
