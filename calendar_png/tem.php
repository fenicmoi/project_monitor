		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class='data-tb5'>
				...
				<input type='hidden' name='save' value='ok'>
				 ...
				 <tr> 
                    ...
                    <td align='right' ><font class='f-black-b'>ผู้บริหาร</font>&nbsp;:&nbsp;</td>

                    <td ><select class='input-select-big' name='manager_id'  >

						<option value=''>-- เลือกผู้บริหาร --</option>

						<?php 

						$res[calen] = $db->select_query("SELECT * FROM ".TB_man." $condi order by manager_name,manager_sname ",MYDBMS);

						while($arr[calen] = $db->fetch($res[calen],MYDBMS)){

							?><option value='<?php echo $arr[calen][manager_id]?>' <?php if($arr[calen][manager_id]==$arr[car][cars_member]) echo "selected";?>><?php echo $arr[calen][manager_pname]?><?php echo $arr[calen][manager_name]?> <?php echo $arr[calen][manager_sname]?> <?php echo $arr[calen][manager_name]?> , <?php echo $dataOth->pos_($arr[calen][manager_pos],$parts);?></option><?php 
				  ...
				  <tr> 
                    ...
                    <td align='right' ><font class='f-black-b'>ตั้งแต่วันที่</font>&nbsp;:&nbsp;</td>

                    <td ><input name="calendar_stdate"  id="start-date" class="date-pick dp-applied"><p class='detailtext'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font class='f-black-b'>ถึงวันที่</font>&nbsp;:&nbsp;</p><input name="calendar_endate" id="end-date" class="date-pick dp-applied"><p class='detailtext'>( <font class='f-red'>กรณี 1 วันไม่จำเป็นต้องเลือกวันสิ้นสุด</font> )</p>
				 ...
				 <tr> 
                    ...
                    <td align='right' ><font class='f-black-b'>ตั้งแต่เวลา</font>&nbsp;:&nbsp;</td>

                    <td ><select class='input-select' name='stH'  >
						...
						}?></select> น. <select class='input-select' name='stM'  >
						...
						}?></select> นาที&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font class='f-black-b'>ถึงเวลา</font>&nbsp;:&nbsp;<select class='input-select' name='enH'  >
						...
						}?></select> น. <select class='input-select' name='enM'  >
				 ...
				 <tr> 
                    ...
                    <td align='right' ><font class='f-black-b'>หัวข้อภารกิจ</font>&nbsp;:&nbsp;</td>

                    <td ><input type='text' name='calendar_title' value='<?php echo $arr[calen][manager_pname]?>' size='80' class='input-text' >
				 ...
				 <tr> 
                    ...
                    <td align='right' valign='top'><font class='f-black-b'>รายละเอียดภารกิจ</font>&nbsp;:&nbsp;</td>

                    <td ><textarea name='calendar_detail' cols='80' rows='5' class='input-textarea'></textarea>
				 ...
				 <tr> 
                    ...
                    <td align='right' ><font class='f-black-b'>สถานที่</font>&nbsp;:&nbsp;</td>

                    <td ><input type='text' name='calendar_location' value='<?php echo $arr[calen][manager_pname]?>' size='60' class='input-text' >
				 ...
				 <tr> 
                    ...
                    <td align='right' ><font class='f-black-b'>เจ้าของเรื่อง / โทรศัพท์</font>&nbsp;:&nbsp;</td>

                    <td ><input type='text' name='calendar_own' value='<?php echo $arr[calen][manager_pname]?>' size='70' class='input-text' >
...
<!-- firebug lite -->
				...
				<div class="row">
					...
					<div class="col-md-6 well">
						...
						<div class="form-group form-inline">
							...
							<select class='form-control' name='manager_id'  >
								...
								while($arr[man] = $db->fetch($res[man],MYDBMS)){
									?><option value='<?php echo $arr[man][manager_id]?>' <?php if($arr[man][manager_id]==$arr[calen][manager_id]) echo "selected";?>><?php echo $arr[man][manager_pname]?><?php echo $arr[man][manager_name]?> <?php echo $arr[man][manager_sname]?> <?php //echo $arr[man][manager_name]?> , <?php echo $dataOth->pos_($arr[man][manager_pos],$parts);?></option><?php 
								}?>
							</select>
						</div>
						...
						<div class="form-group form-inline">
							...
							<?php 
							for($H=0;$H<=23;$H++){
								?><option value='<?php echo $H?>' <?php if($H==$stH) echo "selected";?>><?php echo $H?></option><?php 
							}?>
							</select> น. 
							...
							<?php 
							for($H=0;$H<=59;$H++){
								?><option value='<?php echo $H?>' <?php if($H==$stM) echo "selected";?>><?php echo $H?></option><?php 
							}?>
							...
							<?php 
							for($H=0;$H<=23;$H++){
								?><option value='<?php echo $H?>' <?php if($H==$enH) echo "selected";?>><?php echo $H?></option><?php 
							}?>
							</select> น.
							 <select class='form-control' name='enM'  >
							<?php 
							for($H=0;$H<=59;$H++){
								?><option value='<?php echo $H?>' <?php if($H==$enM) echo "selected";?>><?php echo $H?></option><?php 
							}?></select> นาที	
						</div>
	...
	}else if($_SESSION['job1'] =="listdata" || !$_SESSION['job1']){
		...
		<div style="text-align:left;"><?php $dataPC->pagecheck_normal($perpage,$datalink,$page_c,30,$sumcount,$parts);?></div>
			 ...
			 <table class='table table-bordered table-striped'>
				...
				?>
					...
					<tr <?php if($arr[calen][manager_id]==$id) echo "class='data-edit'";?>>   
						 ...
						 <td  align='center' valign='top'>
							...
							<?php if(strtotime($stdate)!=strtotime($endate)) echo $conDate->DateConvertSH5($arr[calen][calendar_endate],$parts);?> 
