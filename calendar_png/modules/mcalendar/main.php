...
?>
<font class='f-blue-big'>ตารางนัดหมายในปี <?php echo (YENOW+543)?> ของ <?php if(!$manager_id) echo "ผู้บริหารแต่ละท่าน";?> </font> <font class='f-green-big'><?php echo $dataUser->man_($manager_id,$parts)?></font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='calendar/prinpreviewyear.php?nyear=<?php echo YENOW?>&manager_id=<?php echo $manager_id?>' target='_blank'><img src='images/print_version.gif' border='0' ></a>		
<div id='main'>
				<?php
				if(!$manager_id){
					$rec_num=0;
				$res[man] = $db->select_query("SELECT * FROM ".TB_man." $condi1 ORDER BY manager_id",MYDBMS);
				while($arr[man] = $db->fetch($res[man],MYDBMS)){
					$rec_num++;
				?><div class='mandetail'>
						<div class='man-pic'><a href='?manager_id=<?php echo $arr[man][manager_id]?>'><?php if($arr[man][manager_pic]) echo "<img src='managerPic/".$arr[man][manager_pic]."' width='90' border='0'>"; else echo "<img src='images/man.jpg' border='0'>";?></a></div>
						 <div class='man-name'><?php echo $arr[man][manager_pname]?><?php echo $arr[man][manager_name]?> <?php echo $arr[man][manager_sname]?><br>ตำแหน่ง : <?php echo $dataOth->pos_($arr[man][manager_pos],$parts);?></div>
				</div>
