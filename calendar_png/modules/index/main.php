...
<div id='main'>
	...
	<div id='calendar-main'>
		...
		<div id='month-name'>
			...
			<form method=post action="?" name="webForm">
				  ...
				  <tr> 
                    ...
                    <td valign='bottom' align='left' ><select class='input-select-big1' name='manager_id'   onchange='reloadPage()'>

						<option value=''>-- ผู้บริหารทั้งหมด --</option>

						<?php 

						$res[man] = $db->select_query("SELECT * FROM ".TB_man." order by manager_id",MYDBMS);

						while($arr[man] = $db->fetch($res[man],MYDBMS)){

							?><option value='<?php echo $arr[man][manager_id]?>' <?php if($arr[man][manager_id]==$manager_id) echo "selected";?>><?php echo $arr[man][manager_pname]?><?php echo $arr[man][manager_name]?> <?php echo $arr[man][manager_sname]?> <?php //echo $arr[man][manager_name]?> : <?php echo $dataOth->pos_($arr[man][manager_pos],$parts);?></option><?php 

						}?></select>

						

						<select class='input-select-big1' name='smonth'  onchange='reloadPage()'>

						<?php for($month=1;$month<=12;$month++){

							?><option value='<?php echo $month;?>' <?php if($month==$smonth) echo "selected";?>><?php echo $conDate->mon_full($month,"th");?></option><?php 

						}?></select>

						<select class='input-select-big1' name='syear'  onchange='reloadPage()'>
                    ...
                    <td align='left' width='20'><a href='calendar/prinpreview.php?manager_id=<?php echo $_POST['manager_id']?>&smonth=<?php echo $_POST['smonth']?>&syear=<?php echo $_POST['syear']?>' target='_blank'><img src='images/print_version.gif' border='0' ></a></td>
		...
		<div class='sunname'><?php echo $conDate->day_full(0,"th");?></div>
		...
		<div class='satname'><?php echo $conDate->day_full(6,"th");?></div>

		<?php for($day=$stDay;$day<=$endDay;$day++){
